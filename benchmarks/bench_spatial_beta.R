...
file_primary <- file.path("inst/exdata", "spatial_beta_benchmark.rds")
...
for (n_val in D_values) {
  ...
  sim <- sim_area_data(D = n_val, W = W, spatial_type = "knn", seed = 42 + n_val)
  df <- sim[["data"]]
  ...
  cat(sprintf("    [Saved] Updated all RDS files in inst/exdata and inst/extdata (Total rows: %d)\n\n", nrow(df_existing)))
}

cat("==============================================================================\n")
cat("Spatial Beta Benchmark Complete! Final Summary Table:\n")
cat("==============================================================================\n\n")
