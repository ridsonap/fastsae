...
library("fastsae")
data("sim_area")
head(sim_area[, c("area", "x1", "x2", "y_gaussian", "vardir", "y_beta")], 4)
@
%
...
%
<<spatial_matrix>>=
coords <- cbind(sim_area$x_coord, sim_area$y_coord)
dists <- as.matrix(dist(coords))
W <- (dists < quantile(dists, 0.18)) * 1
diag(W) <- 0
# Row-standardize proximity matrix
rs <- rowSums(W)
rs[rs == 0] <- 1
W_std <- W / rs
@
...
%
<<fit_fh>>=
fit_fh <- eblup_fh(y_gaussian ~ x1 + x2, vardir = "vardir",
  data = sim_area, method = "REML", print_result = FALSE)
fit_sfh <- eblup_sfh(y_gaussian ~ x1 + x2, vardir = "vardir",
...
%
<<compare_fh>>=
cat("FH sigma2_u:  ", round(fit_fh$random_effect_var, 4), "\n")
cat("SFH sigma2_u: ", round(fit_sfh$random_effect_var, 4), "\n")
cat("SFH spatial autoregressive rho:", round(fit_sfh$rho, 4), "\n")
@
...
%
<<fit_beta>>=
fit_beta <- ebp_area(y_beta ~ x1 + x2, vardir = "vardir",
...
%
<<estimates_beta>>=
head(fit_beta$df_ebp[, c("domain", "y", "ebp", "sd", "rse", "ci_lower", "ci_upper")], 6)
@
...
diag_beta <- diagnose(fit_beta)

# Inspect Brown et al. Wald test of unbiasedness for FH
diag_fh$brown_test$wald_test

# Inspect Bayesian information criteria and CPO/PIT calibration
cat("Beta WAIC: ", round(diag_beta$bayesian_metrics$waic, 2), "\n")
cat("Beta DIC:  ", round(diag_beta$bayesian_metrics$dic, 2), "\n")
cat("Mean CPO:  ", round(diag_beta$bayesian_metrics$cpo_summary$mean_cpo, 3), "\n")
cat("CPO Fails: ", diag_beta$bayesian_metrics$cpo_summary$failures, "\n")
@
...
\begin{figure}[t!]
\centering
<<plot_diagnostics, echo=TRUE, fig=TRUE, height=5, width=7>>=
print(autoplot(diag_beta))
@
